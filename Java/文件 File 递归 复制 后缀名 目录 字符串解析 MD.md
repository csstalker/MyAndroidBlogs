...
- 通过 `modifyFileSuffix` 递归修改文件的后缀名，同时删除不匹配的文件
- 通过 `getFormatFilesNames` 获取格式化目录，并将内容复制到粘贴板
...
- 通过为知笔记导出指定目录下的所有文件，导出为U8格式的纯文本
- 通过 `modifyFile` 递归修改文件的后缀名，同时删除不匹配的文件，**同时修改文件中的一些不太正常的字符**
- 通过 `getFormatFilesNames` 获取格式化目录，并将内容复制到粘贴板
- 将粘贴板中的内容粘贴到`REANME.MD`中就可以了

```java
modifyFile(new File(DIR));
getFormatFilesNames(new File(DIR));
```
